...
BASE_DIR = Path("./")
PRECISION_DIR = BASE_DIR / "infinite_precision"
WILD_DIR = BASE_DIR / "infinite_wild"
...
# ==================== 8大情報源全域爬蟲探針 ====================
def infinite_precision_loop():
    global TRACK_STATE
    sources = ["github", "huggingface", "reddit", "codeberg", "crates", "npm", "pypi", "lobsters"]
    ...
    while True:
        ...
        try:
            ...
            if chosen_source == "github":
                state["status"] = "【8源探針】掃描 GitHub 開源庫..."
                query = random.choice(["ai agent", "python automation", "LLM", "scraper"])
                url = f"https://api.github.com/search/repositories?q={urllib.request.quote(query)}+language:python&sort=stars&order=desc"
                req = urllib.request.Request(url, headers={'User-Agent': 'Infinite-Bot', 'Accept': 'application/vnd.github.v3+json'})
            ...
            # 2. Hugging Face
                ...
                state["status"] = "【8源探針】掃描 Hugging Face AI 模型..."
                url = "https://huggingface.co/api/models?sort=downloads&direction=-1&limit=5"
                req = urllib.request.Request(url, headers={'User-Agent': 'Infinite-Bot'})
                with urllib.request.urlopen(req, timeout=8) as resp:
            ...
            # 3. Reddit
                ...
                state["status"] = "【8源探針】掃描 Reddit 開源熱門板..."
                ...
                with urllib.request.urlopen(req, timeout=8) as resp:
                    ...
                    if posts:
                        ...
                        title_slug = "".join(c for c in target.get("title", "post")[:25] if c.isalnum() or c=='_')
                        item_id = f"RD_{title_slug}"
                        item_data = f"Reddit: {target.get('url')}"

            # 4. Codeberg
            elif chosen_source == "codeberg":
                state["status"] = "【8源探針】掃描 Codeberg 開源庫..."
            ...
            # 5. Crates.io (Rust)
            elif chosen_source == "crates":
                state["status"] = "【8源探針】掃描 Crates.io (Rust系統庫)..."
                ...
                with urllib.request.urlopen(req, timeout=8) as resp:
                    ...
                    if crates:
                        target = random.choice(crates)
                        item_id = f"CR_{target['name']}"
                        item_data = f"Crates.io: https://crates.io/crates/{target['name']}"

            # 6. npm (Node.js)
            elif chosen_source == "npm":
                state["status"] = "【8源探針】掃描 npm (前端/工具庫)..."
                url = "https://registry.npmjs.org/-/v1/search?text=automation&size=5"
                req = urllib.request.Request(url, headers={'User-Agent': 'Infinite-Bot'})
                with urllib.request.urlopen(req, timeout=8) as resp:
                    data = json.loads(resp.read().decode('utf-8'))
                    objects = data.get("objects", [])
                    if objects:
                        target = random.choice(objects).get("package", {})
                        item_id = f"NPM_{target.get('name', 'pkg').replace('/', '_')}"
                        item_data = f"NPM: {target.get('links', {}).get('repository', 'https://npmjs.com')}"

            # 7. PyPI (Python)
            elif chosen_source == "pypi":
                state["status"] = "【8源探針】掃描 PyPI (Python官方庫)..."
                # 利用 PyPI 熱門 RSS 或簡單官方 API 替代
                url = "https://pypi.org/pypi/requests/json"
                req = urllib.request.Request(url, headers={'User-Agent': 'Infinite-Bot'})
                with urllib.request.urlopen(req, timeout=8) as resp:
                    data = json.loads(resp.read().decode('utf-8'))
                    info = data.get("info", {})
                    item_id = f"PYPI_requests_lib"
                    item_data = f"PyPI: {info.get('home_page', 'https://pypi.org')}"

            # 8. Lobsters (硬核技術社群)
            elif chosen_source == "lobsters":
                state["status"] = "【8源探針】掃描 Lobsters 技術社群..."
                ...
                with urllib.request.urlopen(req, timeout=8) as resp:
                    ...
                    if stories:
                        ...
                        title_slug = "".join(c for c in target.get("title", "story")[:25] if c.isalnum() or c=='_')
                        item_id = f"LB_{title_slug}"
                        item_data = f"Lobsters: {target.get('url')}"

            # 防重與銷毀審判機制
            if item_id and item_data:
                target_file = PRECISION_DIR / f"{item_id}.txt"
                if target_file.exists():
                    state["status"] = f"【防重銷毀】發現重複物種，當場清除"
                    ...
                    state["status"] = f"【8源收編】成功捕獲並歸檔: {item_id}"
...
def infinite_wild_loop():
    ...
    while True:
        ...
        try:
            state["heartbeat"] += 1
            state["status"] = f"【多維突變】正在進行跨界基因重組..."
            
            chimera_id = f"Omni_Chimera_{random.randint(10000, 99999)}"
            w_file = WILD_DIR / f"{chimera_id}.txt"
            
            if random.random() < 0.8:
                w_file.write_text(f"Omni-mutated organism born at heartbeat #{state['heartbeat']}", encoding="utf-8")
                state["mutations_born"] += 1
                state["energy"] = min(150, state["energy"] + 5)
                state["status"] = f"【右側突變】成功誕生全域新物種！"
            else:
                if w_file.exists(): w_file.unlink()
                state["energy"] = max(20, state["energy"] - 3)
                state["status"] = f"【右側銷毀】基因排斥，殘骸已清除"
...
# ==================== 網頁即時儀表板 ====================
class InfiniteDashboardHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        p_btn_action, p_btn_text, p_btn_color = ("resume", "喚醒8源探針", "#00ffcc") if p["is_paused"] else ("pause", "暫停8源探針", "#ffbb00")
        w_btn_action, w_btn_text, w_btn_color = ("resume", "喚醒多維突變", "#ff00aa") if w["is_paused"] else ("pause", "暫停多維突變", "#ffbb00")
        ...
        <head>
            ...
            <title>8-Source Infinite Bio-Lab</title>
            <style>
        ...
        <body>
            ...
            <div class="container">
                <div class="dish-card" style="border-color: #00ffcc55;">
                    <h3 style="color: #00ffcc; margin: 0 0 10px 0;">🟢 左側：8源育種 (GH/HF/RD/CB/CR/NPM/PY/LB)</h3>
                ...
                <div class="dish-card" style="border-color: #ff00aa55;">
                    <h3 style="color: #ff00aa; margin: 0 0 10px 0;">🟣 右側：多維突變區</h3>
...
if __name__ == "__main__":
    threading.Thread(target=infinite_precision_loop, daemon=True).start()
    threading.Thread(target=infinite_wild_loop, daemon=True).start()
    
    server = HTTPServer(('127.0.0.1', 8080), InfiniteDashboardHandler)
    print("🌌 8源全域實驗室已啟動！請在瀏覽器打開: http://127.0.0.1:8080")
