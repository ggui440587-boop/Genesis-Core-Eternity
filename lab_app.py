...
<head>
    ...
    <style>
        ...
        .btn { background: #d21796; color: white; border: none; padding: 8px 16px; border-radius: 6px; font-family: monospace; font-weight: bold; font-size: 0.85rem; cursor: pointer; width: 100%; margin-top: 6px; }
...
<body>
    ...
    <div class="card card-pure">
        ...
        <div class="sub">當前同種最強：<span id="p-best" class="highlight">PURE_AI_G3746_185</span> (戰力:<span id="p-score">93669</span>)</div>
    </div>
    <div class="card card-hybrid">
        <div class="title" style="color: #a371f7;">⚡ 混合融合軌道（產出數: <span id="h-cnt">7679</span>）</div>
        <div class="sub">當前混合最強：<span id="h-best" class="highlight">HYBRID_OMEGA_6163</span> (戰力:<span id="h-score">306707</span>)</div>
        ...
        function update() {
            fetch('/api/status').then(r => r.json()).then(d => {
                ...
                d.leaderboard.forEach(item => {
                    html += '<div class="lb-item"><span class="lb-title">[⚡ ' + item.type + '] ' + item.name + '</span><span>分數:' + item.score + '</span></div>';
                });
