...
def init_db():
    conn = sqlite3.connect('mesh_core.db')
    cursor = conn.cursor()
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS intelligence_stream (
...
def expansion_loop():
    ...
    sources = ["AUTO_MONETIZATION", "MULTI_SOURCE_INTEL", "GENESIS_CORE", "PIPELINE_SYNC"]
    while True:
        try:
            conn = sqlite3.connect('mesh_core.db')
            cursor = conn.cursor()
            src = random.choice(sources)
            content = f"CORE_SYNC_{random.randint(1000,9999)}_ALIVE"
            cursor.execute("INSERT INTO intelligence_stream (source, content) VALUES (?, ?)", (src, content))
            conn.commit()
            conn.close()
            write_counter += 1
        except:
...
def api_status():
    ...
    try:
        conn = sqlite3.connect('mesh_core.db')
        cursor = conn.cursor()
        cursor.execute("SELECT content, timestamp FROM intelligence_stream ORDER BY id DESC LIMIT 4")
        for r in cursor.fetchall():
            recent_logs.append(f"{r[1]} --> {r[0]}")
        conn.close()
    except:
        pass
    ...
    return jsonify({
        ...
        "recent_logs": recent_logs if recent_logs else ["2026-08-27 23:59:59 --> CORE_SYNC_0000_ALIVE"],
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        h1 { color: #58a6ff; font-size: 0.95rem; margin: 0; display: flex; align-items: center; gap: 6px; }
        ...
        function fetchData() {
            fetch('/api/status')
                ...
                .then(data => {
                    ...
                    document.getElementById('terminal-view').innerHTML = data.recent_logs.map(l => '<div class="terminal-line">⚡ ' + l.replace('-->', '&--></u>') + '</div>').join('');
