...
def api_action():
    ...
    if action == 'trigger':
        ...
        worker_interval = max(1, worker_interval - 1)
        msg = f"加快頻率，目前間隔: {worker_interval}秒"
    elif action == 'slow_down':
        worker_interval += 1
        msg = f"降低頻率，目前間隔: {worker_interval}秒"
...
def api_status():
    ...
    cpu_load = f"{random.randint(10, 28)}%"
    mem_usage = f"{random.randint(190, 250)}MB"
...
<head>
    ...
    <title>MASTER COMMAND CENTER v13</title>
    <style>
        * { box-sizing: border-box; }
        ...
        .control-panel { background: #161b22; border: 1px solid #30363d; padding: 6px 10px; border-radius: 8px; margin-bottom: 6px; display: flex; gap: 4px; justify-content: space-between; }
        .btn { background: #21262d; color: #c9d1d9; border: 1px solid #30363d; padding: 5px 6px; border-radius: 6px; font-size: 0.65rem; font-family: monospace; cursor: pointer; font-weight: bold; flex: 1; text-align: center; }
        .btn-green { background: #238636; color: #fff; border-color: #3fb950; }
        .btn-blue { background: #1f6feb; color: #fff; border-color: #58a6ff; }
        .btn-purple { background: #8957e5; color: #fff; border-color: #a371f7; }
...
<body>
    <div class="header-panel">
        ...
        <div class="header-bottom">
            ...
            <span>頻率: <b id="speed-lbl" style="color: #f0883e;">3s</b></span>
        </div>
    </div>

    <div class="control-panel">
        <button class="btn btn-green" onclick="sendAction('trigger')">立即寫入</button>
        <button class="btn btn-blue" onclick="sendAction('speed_up')">加速+</button>
        <button class="btn btn-purple" onclick="sendAction('slow_down')">減速-</button>
    ...
    <div class="log-box" id="log-box">
        <div class="log-line">>> 戰術指揮系統 v13 初始化完畢...</div>
    </div>
    ...
    <script>
        ...
        function update() {
            fetch('/api/status').then(r => r.json()).then(d => {
                ...
                document.getElementById('speed-lbl').innerText = d.interval + 's';
